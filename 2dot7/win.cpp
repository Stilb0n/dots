...
void win::calc()
{
    ...
    a=str.toDouble(&Ok);
    if ((Ok)&&(a<sqrt(3.40282347e+38))) // если введены символы приемлимого типа,то есть числа то происходят дальнейшие вычисления так же если число меньше 1000
    {
        ...
        if ((!str.isEmpty())&&(a<sqrt(3.40282347e+38))) // дополнительная проверка если строка не пустая
        {
